...
const MeetingCreate: React.FC<MeetingCreateProps> = ({ context, initialMeeting, onBack, onCreated, currentUserId }) => {
  ...
  const [location, setLocation] = useState(initialMeeting?.location || context?.location || '');
  const [isCalendarOpen, setIsCalendarOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  
  const [viewDate, setViewDate] = useState(new Date(meetingDate)); 
  ...
  const isHourDisabled = (isoDate: string, hour: number) => {
    if (!isoDate || isEdit) return false; // 수정 시에는 과거나 현재 시간이라도 변경 가능하게 (사용자 편의)
    const now = new Date();
    ...
    if (title.trim() && meetingDate && selectedHour !== null && location.trim()) {
      ...
      try {
        ...
        if (isEdit && initialMeeting) {
          await updateDoc(doc(db, "meetings", initialMeeting.id), meetingData);
          await updateDoc(doc(db, "chats", initialMeeting.id), {
          ...
          meetingData.targetType = context.type;
          meetingData.targetTitle = context.title;
          meetingData.creatorId = currentUserId;
          meetingData.participants = [currentUserId];
  ...
  return (
    <div className="min-h-screen bg-slate-50 animate-in slide-in-from-right duration-300">
      <div className="h-[60px] bg-white border-b border-slate-100 px-6 flex items-center gap-4 max-w-lg mx-auto">
        ...
        <h2 className="text-lg font-black text-slate-900">{isEdit ? '모임 수정하기' : '모임 만들기'}</h2>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-slate-50 via-slate-50/95 to-transparent max-w-lg mx-auto z-40">
            ...
            >
              ...
              {isEdit ? '모임 정보 수정 완료' : (isFormValid ? `${formatMeetingDate(meetingDate)} · ${formatMeetingTime(selectedHour)} 모임 개설` : '정보를 모두 입력해 주세요')}
            </button>
          </div>
        </form>
      </div>
